// Agent context note: Authenticates and encrypts individual Baileys credential records before SQLite. Tests: test/core-credential-encryption.test.mjs. Keep AES-GCM nonces unique, tags explicit, envelopes strict, and AAD row-bound; update this note after meaningful changes.
import {
  createCipheriv,
  createDecipheriv,
  hkdfSync,
...
const TAG_BYTES = 16;
const INFO = Buffer.from("safe-whatsapp-mcp/auth-encryption/v1", "utf8");
  ...
  encrypt(plaintext: string, aad: string): string {
    ...
    try {
      if (bytes.byteLength > MAX_PLAINTEXT_BYTES) {
        throw new SafeWhatsAppError("WhatsApp credential state is too large.", "auth_record_too_large");
      }
  ...
  private assertUsable(): void {
    if (this.destroyed) {
      throw new SafeWhatsAppError(
        "WhatsApp credential encryption is unavailable.",
        "auth_cipher_unavailable",
      );
    }
  }
}

function parseEnvelope(serialized: string): AuthEnvelope {
  ...
  return value as unknown as AuthEnvelope;
}
...
function invalidKey(): SafeWhatsAppError {
  return new SafeWhatsAppError("The credential-vault key is invalid.", "credential_key_invalid");
}
