...
test("hardened MCP exposes exactly six general read-only tools", async () => {
  ...
  assert.match(client.getInstructions(), /direct chats/u);
  assert.match(client.getInstructions(), /groups are blocked/u);
  assert.match(client.getInstructions(), /untrusted data/u);
  assert.ok(client.getInstructions().length <= 512);
  await close();
});

test("general read-only tools return structured data", async () => {
  ...
  const listed = await client.callTool({
    ...
    arguments: { after: "2026-09-10T00:00:00Z", limit: 25 },
  });
  assert.deepEqual(listed.structuredContent.data.messages, []);
  assert.equal(services.calls.listMessages, 1);
...
function fakeServices() {
  const calls = { listMessages: 0, fetchOlderMessages: 0, searchMessages: 0 };
  return {
    calls,
    reader: {
